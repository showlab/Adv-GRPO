...
def train_siglip(
    scorer, head, prompts, external_imgs_pil, generated_imgs_pil,
    optimizer_D, accelerator
):
    """
    使用 SigLIP 特征训练 reward head (hinge loss)
    scorer: SiglipModel (HF)
    head:   MLP head (in_dim = scorer.config.vision_config.hidden_size)
    """
    ...
    # --- SigLIP 官方 preprocess ---
    siglip_mean = [0.5, 0.5, 0.5]
    siglip_std  = [0.5, 0.5, 0.5]

    # SigLIP 输入 resolution（如 so400m-p14-384 = 384）
    image_size = scorer.config.vision_config.image_size
    augment = transforms.ColorJitter(
        brightness=0.1,
        contrast=0.1,
    )
    ...
    # --- forward ---
    scorer.eval()   # 冻住 SigLIP backbone
    head.train()    # 训练 reward head

    with torch.no_grad():
        # ---- 真实图的特征 ----
        real_out = scorer.vision_model(
            pixel_values=pos_imgs.to(torch.float32)
        )
        real_feat = real_out.pooler_output.to(torch.bfloat16)  # [B, D]

        # ---- 生成图的特征 ----
        fake_out = scorer.vision_model(
            pixel_values=neg_imgs.to(torch.float32)
        )
        fake_feat = fake_out.pooler_output.to(torch.bfloat16)  # [B, D]

    # --- head 输出 logits ---
    logits_real = head(real_feat).squeeze(-1)
    logits_fake = head(fake_feat).squeeze(-1)
...
def main(_):
    ...
    if config.discriminator == "stylegan":
        ...
        # --- 加载 SigLIP 模型（原生支持 512 resolution） ---
        scorer = SiglipModel.from_pretrained("google/siglip-base-patch16-512"  ).to(accelerator.device).to(torch.bfloat16)
        scorer.eval()

        # SigLIP 官方处理器（包含 resize=512、normalize）
        processor = SiglipProcessor.from_pretrained("google/siglip-base-patch16-512")

        # --- 获取特征维度（hidden_size = 1152 for so400m） ---
        feat_dim = scorer.config.vision_config.hidden_size

        # --- 定义 head ---
        class SigLipHead(nn.Module):
            def __init__(self, in_dim, hidden_dim=768):   # hidden_dim 稍微加大更匹配 SigLIP
                super().__init__()
        ...
        head = SigLipHead(in_dim=feat_dim).to(accelerator.device)
    # import pdb; pdb.set_trace()
    ...
    while True:
        ...
        rewards_mean = accelerator.gather(samples["rewards"]['siglip_cotrain']).mean()
        external_rewards_mean = accelerator.gather(samples["external_rewards"]['siglip_cotrain']).mean()
        ...
        # if config.train_d and external_rewards_mean < rewards_mean:
            ...
            d_loss ,acc = train_siglip(scorer, head, prompts_per_gpu, external_imgs_pil, generated_imgs_pil, optimizer_D, accelerator)
