...
def train_pickscore(scorer, prompts, real_data, fake_data, optimizer, accelerator):
    ...
    batch = {
            ...
            # "num_examples_per_prompt" :torch.tensor([8, 8, 8, 8, 8, 8], device=accelerator.device)
        }
    # import pdb; pdb.set_trace()
    loss = criterion(scorer.model, batch)
...
def main(_):
    ...
    scorer.model = DDP(scorer.model, device_ids=[local_rank], output_device=local_rank)
    optimizer_D = torch.optim.Adam(scorer.model.parameters(), lr=config.d_lr, betas=(0.5, 0.999))
    ...
    while True:
        ...
        for p in scorer.model.parameters():
            p.requires_grad = False

        for p in scorer.model.vision_model.encoder.layers[config.tune_layer:].parameters():
