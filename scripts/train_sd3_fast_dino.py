...
def train_dino(scorer, head, prompts, external_imgs_pil, generated_imgs_pil, optimizer_D, accelerator):
    """
    单次训练 DINOv2+Head 判别器的 step
    
    Args:
        scorer: DINOv2 backbone (timm model)，通常冻结
        head: MLP Head，输出 [B,1] logits
        prompts: 文本 prompt 列表（这里暂时没用到，可以扩展做条件判别）
        external_imgs_pil: List[PIL.Image]，正样本（Qwen / 高质量参考）
        generated_imgs_pil: List[PIL.Image]，负样本（SD3 生成图）
        optimizer_D: torch optimizer
        accelerator: accelerate.Accelerator，用于分布式训练
        
    Returns:
        d_loss.item(): float，判别器 loss
        acc: float，训练准确率
    """
    import torch
    ...
    transform = transforms.Compose([
        ...
        transforms.Normalize(mean=[0.485, 0.456, 0.406],
                            std=[0.229, 0.224, 0.225])  # DINOv2 官方预处理
    ])
    ...
    # 标签
    pos_labels = torch.ones(len(pos_imgs), device=device)
    neg_labels = torch.zeros(len(neg_imgs), device=device)

    imgs = torch.cat([pos_imgs, neg_imgs], dim=0)        # [B,3,H,W]
    labels = torch.cat([pos_labels, neg_labels], dim=0)  # [B]
    ...
    with torch.no_grad():
        emb_real = scorer(pos_imgs)
        emb_fake = scorer(neg_imgs)

    logits_real = head(emb_real).squeeze(-1)
    logits_fake = head(emb_fake).squeeze(-1)
    ...
    acc_real = preds_real.mean().item()   # 相当于和全1比

    # fake 样本预测 < 0 判为假
    preds_fake = (logits_fake < 0).float()
    acc_fake = preds_fake.mean().item()   # 相当于和全0比
...
def main(_):
    ...
    while True:
        ...
        rewards_mean = accelerator.gather(samples["rewards"]['dino_cotrain']).mean()
        external_rewards_mean = accelerator.gather(samples["external_rewards"]['dino_cotrain']).mean()
